...
pub unsafe extern "C" fn hsn_lod_dist_sq(
    ...
    let mut i = 0;
    while i + 4 <= len {
        let dx0 = xs[i] - ox; let dy0 = ys[i] - oy; let dz0 = zs[i] - oz;
        let dx1 = xs[i + 1] - ox; let dy1 = ys[i + 1] - oy; let dz1 = zs[i + 1] - oz;
        let dx2 = xs[i + 2] - ox; let dy2 = ys[i + 2] - oy; let dz2 = zs[i + 2] - oz;
        let dx3 = xs[i + 3] - ox; let dy3 = ys[i + 3] - oy; let dz3 = zs[i + 3] - oz;
        dst[i] = dx0 * dx0 + dy0 * dy0 + dz0 * dz0;
        dst[i + 1] = dx1 * dx1 + dy1 * dy1 + dz1 * dz1;
        dst[i + 2] = dx2 * dx2 + dy2 * dy2 + dz2 * dz2;
        dst[i + 3] = dx3 * dx3 + dy3 * dy3 + dz3 * dz3;
        i += 4;
    }
    while i < len {
        let dx = xs[i] - ox;
...
/// Packed Horizon Y. out[i]=1 when max_y[i] < floor.
#[no_mangle]
pub unsafe extern "C" fn hsn_horizon_y_f32(
    max_y: *const f32,
    floor: f32,
    out: *mut u8,
    n: usize,
) {
    if max_y.is_null() || out.is_null() || n == 0 {
        return;
    }
    let ys = std::slice::from_raw_parts(max_y, n);
    let dst = std::slice::from_raw_parts_mut(out, n);
    let mut i = 0;
    while i + 8 <= n {
        dst[i] = if ys[i] < floor { 1 } else { 0 };
        dst[i + 1] = if ys[i + 1] < floor { 1 } else { 0 };
        dst[i + 2] = if ys[i + 2] < floor { 1 } else { 0 };
        dst[i + 3] = if ys[i + 3] < floor { 1 } else { 0 };
        dst[i + 4] = if ys[i + 4] < floor { 1 } else { 0 };
        dst[i + 5] = if ys[i + 5] < floor { 1 } else { 0 };
        dst[i + 6] = if ys[i + 6] < floor { 1 } else { 0 };
        dst[i + 7] = if ys[i + 7] < floor { 1 } else { 0 };
        i += 8;
    }
    while i < n {
        dst[i] = if ys[i] < floor { 1 } else { 0 };

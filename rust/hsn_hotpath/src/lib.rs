...
fn cpu_bits() -> u32 {
    let cached = CPU_CACHE.load(Ordering::Relaxed);
    if cached & 0x80 != 0 {
        return cached;
    }
    let mut bits = 0x80u32;
    ...
    CPU_CACHE.store(bits, Ordering::Relaxed);
    bits
}
...
fn cull_xyz_scalar(
    ...
    while i < n {
        ...
        let dy = y[i] - oy;
        let dz = z[i] - oz;
        out[i] = u8::from(dx * dx + dy * dy + dz * dz > limit_sq);
        i += 1;
    }
}
...
unsafe fn cull_xyz_avx2(
    ...
    use std::arch::x86_64::*;
    let n = x.len().min(y.len()).min(z.len()).min(out.len());
    let vx0 = _mm256_set1_pd(ox);
    let vy0 = _mm256_set1_pd(oy);
    let vz0 = _mm256_set1_pd(oz);
    let vlim = _mm256_set1_pd(limit_sq);
    let mut i = 0;
    while i + 4 <= n {
        let dx = _mm256_sub_pd(_mm256_loadu_pd(x.as_ptr().add(i)), vx0);
        let dy = _mm256_sub_pd(_mm256_loadu_pd(y.as_ptr().add(i)), vy0);
        let dz = _mm256_sub_pd(_mm256_loadu_pd(z.as_ptr().add(i)), vz0);
        let d2 = _mm256_fmadd_pd(dz, dz, _mm256_fmadd_pd(dy, dy, _mm256_mul_pd(dx, dx)));
        let mask = _mm256_movemask_pd(_mm256_cmp_pd(d2, vlim, _CMP_GT_OQ));
        out[i] = (mask & 1) as u8;
        out[i + 1] = ((mask >> 1) & 1) as u8;
        out[i + 2] = ((mask >> 2) & 1) as u8;
        out[i + 3] = ((mask >> 3) & 1) as u8;
...
#[no_mangle]
pub unsafe extern "C" fn hsn_cull_xyz(
    x: *const f64,
    y: *const f64,
    z: *const f64,
    ox: f64,
    ...
    if x.is_null() || y.is_null() || z.is_null() || out.is_null() || len == 0 {
        return;
    }
    let xs = std::slice::from_raw_parts(x, len);
    let ys = std::slice::from_raw_parts(y, len);
    let zs = std::slice::from_raw_parts(z, len);
    let output = std::slice::from_raw_parts_mut(out, len);
    match selected_kernel() {
        Kernel::Avx512 => {
            #[cfg(hsn_has_avx512_obj)]
            {
                hsn_cull_xyz_avx512_impl(x, y, z, ox, oy, oz, limit_sq, out, len);
                return;
            }
            #[cfg(not(hsn_has_avx512_obj))]
            {
                if cpu_has_avx2() {
                    #[cfg(target_arch = "x86_64")]
                    {
                        cull_xyz_avx2(xs, ys, zs, ox, oy, oz, limit_sq, output);
                        return;
                    }
                }
                cull_xyz_scalar(xs, ys, zs, ox, oy, oz, limit_sq, output);
            }
        }
        Kernel::Avx2 => {
            #[cfg(target_arch = "x86_64")]
        ...
        Kernel::Scalar => cull_xyz_scalar(xs, ys, zs, ox, oy, oz, limit_sq, output),
    }
}

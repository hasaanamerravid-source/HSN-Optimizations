...
static void cull_aabb_scalar(const float *planes24, const float *aabb6, uint8_t *out, size_t n) {
    for (size_t i = 0; i < n; i++) {
        ...
        out[i] = (uint8_t)(
            plane_rejects(planes24, minx, miny, minz, maxx, maxy, maxz)
            || plane_rejects(planes24 + 4, minx, miny, minz, maxx, maxy, maxz)
            || plane_rejects(planes24 + 8, minx, miny, minz, maxx, maxy, maxz)
            || plane_rejects(planes24 + 12, minx, miny, minz, maxx, maxy, maxz)
            || plane_rejects(planes24 + 16, minx, miny, minz, maxx, maxy, maxz)
            || plane_rejects(planes24 + 20, minx, miny, minz, maxx, maxy, maxz));
    }
}

...
public final class HotPath {
	...
	public static void rebuild(HSNConfig cfg) {
		...
		flags = bits;
		// Cache the un-squared distance too: mixins previously called Math.sqrt()
		// on this value once per spawned particle. Distance limits only change on
		// config save / scale change, so compute the root once here instead.
		particleDistBase = cfg.maxParticleDistance;

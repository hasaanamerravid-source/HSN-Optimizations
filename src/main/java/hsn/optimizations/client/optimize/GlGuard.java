...
/**
 * Minecraft 26.3 can run the Vulkan backend. There is no GL context then.
...
public final class GlGuard {
	...
	public static boolean glUsable() {
		...
		try {
			Class.forName("org.lwjgl.opengl.GL").getMethod("getCapabilities").invoke(null);
	...
	private static long currentContext() {
		try {
			return GlCompat.currentNativeContext();
		} catch (Throwable ignored) {
			return 0L;
		}
	}

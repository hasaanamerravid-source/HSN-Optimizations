...
public final class LowEndTuner {
	...
	public static void noteGpu(String renderer, String vendor) {
		...
		boolean irisXe = r.contains("iris xe") || r.contains("iris plus")
				|| r.contains("intel iris") || (r.contains("iris") && v.contains("intel"));
	...
	public static void tick() {
		...
		if (GpuProbe.resolved()) {
			probed = true;
			IntegratedGpuGuard.applyIfNeeded();
		} else if (!probed) {
			probeGpu();
		}
		...
		if (cfg.laptopPowerSaveEnabled && onBattery()) {
			budget = Math.min(budget, Math.max(0.55, scale));
		} else if (cfg.laptopPowerSaveEnabled && AdaptiveCuller.getSmoothedFps() < cfg.targetFps) {
			budget = Math.min(budget, Math.max(0.70, scale));
		}
	...
	private static String gl(int pname) {
		...
		return GlCompat.getString(pname);
	}

	private static volatile long batteryCheckedAt;
	private static volatile boolean batteryCached;

	static boolean onBattery() {
		long now = System.currentTimeMillis();
		if (now - batteryCheckedAt < 5_000L) {
			return batteryCached;
		}
		batteryCheckedAt = now;
		batteryCached = readBatteryDischarging();
		return batteryCached;
	}

	private static boolean readBatteryDischarging() {
		try {
			java.nio.file.Path root = java.nio.file.Path.of("/sys/class/power_supply");
			...
			try (var stream = java.nio.file.Files.list(root)) {
				return stream.anyMatch(p -> {
					...
					try {
						String status = java.nio.file.Files.readString(p.resolve("status")).trim();
						return "Discharging".equalsIgnoreCase(status);

...
public final class RenderShapeCuller {
	...
	private static final int MIN_KEEP_CHUNKS = 3;
	private static final int MAX_SECTION_CAP = 4096;
	...
	private static void ensure(int n) {
		...
		int cap = Math.max(n, dropBuf.length == 0 ? 64 : dropBuf.length * 2);
		if (cap < 64) {
			cap = 64;
		}
		if (cap > MAX_SECTION_CAP && n <= MAX_SECTION_CAP) {
			cap = MAX_SECTION_CAP;
		}

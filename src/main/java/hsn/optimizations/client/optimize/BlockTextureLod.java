...
public final class BlockTextureLod {
	...
	public static void captureBoundAtlas() {
		if (!GraphicsBackend.textureLodSupported()) {
			return;
		}
		try {
	...
	private static void apply(float bias) {
		if (atlasGlId <= 0 || !GraphicsBackend.textureLodSupported() || !GlGuard.glUsable()) {
			return;
		}
		if (lastApplied == bias) {
			return;
		...
		try {
			...
			texParameterFloat(GL_TEXTURE_2D, GL_TEXTURE_LOD_BIAS, bias);
			if (bias > 0.0f && (HSNConfig.get().performanceModeEnabled || AdaptiveCuller.isWeakGpuActive())) {
				texParameterFloat(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY, 1.0f);
			}
			if (prev > 0) {
	...
	/**
	 * 26.2 GlStateManager._texParameter is int-only on some mappings.
	 * LOD bias and anisotropy need a float; pick the float overload by name.
	 */
	private static void texParameterFloat(int target, int pname, float value) {
		String[] names = {"_texParameter", "_texParameterf", "texParameter", "texParameterf"};
		for (String name : names) {
			try {
				GlStateManager.class.getMethod(name, int.class, int.class, float.class)
		...
		try {
			org.lwjgl.opengl.GL11.glTexParameterf(target, pname, value);
		} catch (Throwable ignored) {
		}
	}
}

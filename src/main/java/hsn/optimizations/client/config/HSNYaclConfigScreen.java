...
public final class HSNYaclConfigScreen {
	...
	private static ConfigCategory rendering(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				...
				.tooltip(Component.literal("World scale, terrain mask, LOD, textures, fog, sky."))
				.group(OptionGroup.createBuilder()
						.name(Component.literal("World scale").withStyle(ChatFormatting.AQUA))
						.option(toggle("World scale", () -> cfg.renderScaleEnabled, v -> cfg.renderScaleEnabled = v, false,
								"3D framebuffer scale only. HUD stays native. Forced off when Resolution Control++ is present. Sky disc stays on the native framebuffer.",
								"Default: off."))
						.option(percent("Scale percent", () -> cfg.renderScale, v -> cfg.renderScale = v, 100, 25, 200,
						...
						.option(Option.<ScaleFilter>createBuilder()
								...
								.build())
						.option(toggle("Adaptive scale", () -> cfg.renderScaleAdaptive, v -> cfg.renderScaleAdaptive = v, false,
	...
	private static ConfigCategory extra(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Extra").withStyle(ChatFormatting.GOLD))

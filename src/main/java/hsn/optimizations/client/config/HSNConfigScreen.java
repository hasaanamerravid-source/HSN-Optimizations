...
public final class HSNConfigScreen {
	...
	public static Screen create(Screen parent) {
		...
		var builder = YetAnotherConfigLib.createBuilder()
				...
				.category(HSNConfigSearch.category(cfg, parent))
				.category(simple(cfg))
				.category(general(cfg))
				.category(rendering(cfg))
				.category(compat(cfg))
				.save(cfg::save);
		HSNConfigSearch.attachScreenInit(builder);
		return builder.build().generateScreen(parent);
	}


	private static ConfigCategory simple(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Simple").withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD))
				.tooltip(Component.literal("The short page. Profile + a few sliders. Everything else is in Search."))
				.option(label("Most people only need this page",
						IrisCompat.loaded()
								? "Shader pack: " + IrisCompat.statusLine()
								: "No shader mod detected. 3D scale and lightmap cache may run."))
				.option(Option.<Preset>createBuilder()
						.name(Component.literal("How fast should it look?"))
						.description(desc(
								"Writes every slider at once.",
								"Ultra Low — oldest laptops. Safe — still fast but readable. Balanced — default. Quality — almost vanilla. Competitive — long view, less CPU waste.",
								"Move toward Ultra Low for FPS. Move toward Quality for looks."))
						.instant(true)
						...
						.build())
				.option(toggle("HSN On", () -> cfg.modEnabled, v -> cfg.modEnabled = v, true,
						"Master switch. Off = vanilla / Sodium only.",
						"Same as the kill switch."))
				.option(toggle("Performance Mode", () -> cfg.performanceModeEnabled, v -> cfg.performanceModeEnabled = v, false,
						"Short distances and tighter particle budget right now.",
						"Use for raids and huge bases."))
				.option(toggle("Shader Safe Mode", () -> cfg.shaderSafeMode, v -> cfg.shaderSafeMode = v, true,
						"When a shader pack is on, HSN will not resize the world framebuffer or freeze the lightmap. That is what made BSL look like day at night and put shadows in the wrong place.",
						"Leave on if you use BSL, Complementary, or any Iris pack.", FPS_NONE))
				.option(slider("Mob Distance", () -> (int) cfg.maxEntityRenderDistance, v -> cfg.maxEntityRenderDistance = v, 32, 8, 128, "blocks",
						"How far living mobs stay visible.",
						"Lower = faster."))
				.option(slider("Particle Distance", () -> (int) cfg.maxParticleDistance, v -> cfg.maxParticleDistance = v, 16, 4, 128, "blocks",
						"How far splash / smoke / sparkles stay visible.",
						"Lower = faster."))
				.option(toggle("World Resolution Scale", () -> cfg.renderScaleEnabled, v -> {
							cfg.renderScaleEnabled = v;
							RenderScale.notifyChanged();
						}, false,
						"Draw the world at a lower resolution than the hotbar. Off while a shader pack is bound.",
						"50–75% helps integrated GPUs. 100% is vanilla."))
				.option(slider("World Resolution", () -> (int) Math.round(cfg.renderScale * 100), v -> {
							cfg.renderScale = v / 100.0;
							RenderScale.notifyChanged();
						}, 100, 25, 200, "%",
						"100% is native. 50% is half-res world (more FPS).",
						"Sky stays on the native buffer. Shaders force this off."))
				.option(toggle("FPS Counter", () -> cfg.fpsOverlayEnabled, v -> cfg.fpsOverlayEnabled = v, false,
						"Tiny FPS line on screen without opening F3.",
						"Off by default.", FPS_ON_LOW))
				.build();
	}

	private static ConfigCategory general(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Start here").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("Pick a profile, flip the kill switch, done. Other tabs are optional."))
				.option(label("HSN-Optimizations " + HSNConfig.modVersionLabel + "  ·  Fabric 26.2",
						"Sits next to Sodium. It does not replace Sodium, Lithium, or Entity Culling.\n\n"
								+ "1. Pick a Profile.  2. Play.  3. Open Search if you want one slider.\n\n"
								+ "Keys start unbound — Options → Controls → HSN Optimizations."))
				.option(Option.<Preset>createBuilder()
						.name(Component.literal("Profile"))
						.description(desc(
								"Writes every gameplay slider and toggle in this menu. Compatibility deferrals and overlay position stay as you left them.",
				...
				.group(OptionGroup.createBuilder()
						...
						.option(toggle("HSN Enabled (kill switch)", () -> cfg.modEnabled, v -> cfg.modEnabled = v, true,
								...
								"Optional. Bind the kill switch and settings keys in Options → Controls → HSN Optimizations. Both start unbound. Use the Search tab to jump to a setting by name."))
						.option(toggle("Shader Safe Mode", () -> cfg.shaderSafeMode, v -> cfg.shaderSafeMode = v, true,
								"Turns off 3D scale, lightmap cache, fog scale, horizon cut and cloud/sky skips while an Iris pack is bound.",
								"Leave on for BSL / Complementary. Off only if you are debugging."))
						.option(toggle("Performance Mode", () -> cfg.performanceModeEnabled, v -> cfg.performanceModeEnabled = v, false,
	...
	private static ConfigCategory culling(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Distance").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("How far particles, mobs, chests and sounds stay visible. Shorter = faster."))
				.group(OptionGroup.createBuilder()
				...
				.group(OptionGroup.createBuilder()
						...
						.option(slider("Entity Distance", () -> (int) cfg.maxEntityRenderDistance, v -> cfg.maxEntityRenderDistance = v, 32, 4, 256, "blocks",
								"Exact draw distance for living entities, in blocks, measured from the camera to the entity's bounding box. 8 means the mob stays visible until you are more than 8 blocks away. No hidden scale, no second square, no priority shrink.",
								"Default 32 blocks. Adaptive Culling is the only option that may shrink this, and only while that toggle is on."))
						...
						.option(toggle("Behind-Camera Cull", () -> cfg.behindCameraCullEnabled, v -> cfg.behindCameraCullEnabled = v, false,
								"Skip living entities that are behind the camera and farther than half the entity slider. Nearby mobs beside or behind you still render.",
								"Default: off. Turn on only if you want Entity-Culling-style back-face hiding."))
	...
	private static ConfigCategory rendering(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Picture").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("World resolution, textures, fog and terrain shape. This is the 'how it looks' tab."))
				.group(scaleGroup(cfg))
				.group(OptionGroup.createBuilder()
				...
				.group(OptionGroup.createBuilder()
						...
						.option(slider("Vertical Range", () -> cfg.circularVerticalRange, v -> cfg.circularVerticalRange = v, 16, 4, 64, "blocks",
								...
								"Default 16 blocks."))
						.option(toggle("Horizon Y Cull", () -> cfg.horizonYCullEnabled, v -> {
									cfg.horizonYCullEnabled = v;
									HotPath.rebuild();
								}, true,
								"Each frame, find the lowest Y the camera can see from pitch, FOV and view distance. Sections, entities and particles entirely below that floor are not drawn.",
								"Default: on. Looking down keeps caves. Looking at the horizon from high up drops deep terrain."))
						.option(slider("Horizon Keep-Below", () -> (int) cfg.horizonYKeepBelow, v -> cfg.horizonYKeepBelow = v, 24, 8, 96, "blocks",
								"Blocks kept under the camera so the ground at your feet cannot disappear while Horizon Y Cull is on.",
								"Default 24 blocks. Lower values hide more caves and raise FPS."))
	...
	private static ConfigCategory performance(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Speed").withStyle(ChatFormatting.GOLD))
				...
				.group(OptionGroup.createBuilder()
						...
						.option(Option.<SimdMode>createBuilder()
								...
								.instant(true)
								.binding(SimdMode.AUTO, () -> cfg.simdMode, v -> {
									cfg.simdMode = v;
									HotPath.rebuild();
								})
	...
	private static ConfigCategory highEnd(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Extras").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("CPU-side cuts that still help when the GPU is a 5090-class card."))
				.option(label("3.8.8 R high-end pass",
				...
				.group(OptionGroup.createBuilder()
						...
						.option(toggle("Sky Extras Throttle", () -> cfg.skyExtrasThrottleEnabled, v -> cfg.skyExtrasThrottleEnabled = v, true,
								...
								"Recommended: on."))
						.option(toggle("Ambient Block Ticks", () -> cfg.ambientTickCullEnabled, v -> cfg.ambientTickCullEnabled = v, true,
								"Throttles the client random-block walk that spawns torch flame, drip, spores and furnace crackle every tick. Vanilla scans a 16-block cube 20 times a second. This does not change real block ticks, redstone or combat.",
								"Recommended: on. One of the largest client CPU cuts after entity culling."))
						.option(slider("Ambient Tick Interval", () -> cfg.ambientTickInterval, v -> cfg.ambientTickInterval = v, 2, 1, 8, "ticks",
								"Run the ambience walk every N client ticks. 1 is vanilla. 2 halves the work. 4 is the Ultra Low default.",
						...
						.option(slider("Ambient Tick Range", () -> cfg.ambientTickRange, v -> cfg.ambientTickRange = v, 12, 4, 16, "blocks",
								"How far from the player random ambience may spawn. Vanilla is 16. Lower values skip far torch/drip noise the camera never sees.",
								"Default 12 blocks. Lower is faster.", FPS_LOWER_LOW))
	...
	private static ConfigCategory advanced(HSNConfig cfg) {
		return ConfigCategory.createBuilder()
				.name(Component.literal("Tweaks").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("Particle keep-rates, F3 lines, and singleplayer simulation extras."))
				.group(OptionGroup.createBuilder()
				...
				.group(OptionGroup.createBuilder()
						...
						.option(toggle("F3 Compact", () -> cfg.f3Compact, v -> cfg.f3Compact = v, true,
								"Keeps the right-column HSN block to two short lines. Turn off for the older multi-line dump.",
								"Display only. Does not change world rendering.", FPS_NONE))
						.option(toggle("F3 Details", () -> cfg.f3ShowDetails, v -> cfg.f3ShowDetails = v, true,
								"Adds live distances, feature flags, VoxelSniper state, and render-scale on a third F3 line.",
								"Display only. Does not change world rendering.", FPS_NONE))
						.option(toggle("Skip Empty Boss Overlay", () -> cfg.skipEmptyBossOverlayEnabled, v -> cfg.skipEmptyBossOverlayEnabled = v, true,
								"Skips boss-bar layout work when no boss events are active.",
								"Default: on. Cheap no-op guard.", FPS_ON_LOW))
						.option(toggle("FPS Overlay", () -> cfg.fpsOverlayEnabled, v -> cfg.fpsOverlayEnabled = v, false,
								"Draws a compact on-screen FPS readout. Independent of F3. The settings keybind starts unbound. Use Mod Menu or bind it in Controls.",
				...
				.group(OptionGroup.createBuilder()
						...
						.option(slider("Locate Cache TTL", () -> cfg.locateCacheTtlSeconds, v -> cfg.locateCacheTtlSeconds = v, 30, 5, 120, "s",
						 ...
						 .option(toggle("Instant World Open", () -> cfg.worldOpenEnabled, v -> cfg.worldOpenEnabled = v, true,
								"Prefetches region files, drains a time-boxed amount of chunk work after start, and admits play when the local player exists. Does not rewrite view distance.",
								"Default: on. World-open only.", FPS_NONE))
						.option(slider("World-Open Window", () -> cfg.fastWorldLoadWindowSeconds, v -> cfg.fastWorldLoadWindowSeconds = v, 8, 1, 20, "s",
								"How many seconds after server start extra chunk drain stays active.",
								"Default 8 seconds. Join-time only.", FPS_NONE))
						.option(slider("World-Open Drain", () -> cfg.fastWorldLoadChunkBoost, v -> cfg.fastWorldLoadChunkBoost = v, 12, 1, 16, "",
								"Extra chunk tasks drained per tick during the open window.",
								"Default 12. Join-time only.", FPS_NONE))
						.option(toggle("Admit Play When Ready", () -> cfg.admitPlayWhenReady, v -> cfg.admitPlayWhenReady = v, true,
								"Singleplayer / LAN only. On a remote server vanilla keeps the Joining world screen until chunks arrive.",
								"Default: on for integrated worlds. Multiplayer handshake is left alone.", FPS_NONE))
						.option(toggle("Prefetch Region Files", () -> cfg.prefetchRegionFiles, v -> cfg.prefetchRegionFiles = v, true,
								"C kernel (Java fallback) warms region / entity / poi files so the first disk reads hit page cache.",
								"Default: on.", FPS_NONE))
						.option(toggle("Frame-First Scheduler", () -> cfg.schedulerEnabled, v -> cfg.schedulerEnabled = v, true,
								"Raises render-thread priority and leaves CPU headroom from real core/cgroup counts. Does not fake availableProcessors().",
								"Default: on."))
						.option(toggle("Smart Yield", () -> cfg.smartYieldEnabled, v -> cfg.smartYieldEnabled = v, true,
								"The client loop yields only when the last frame already missed 10ms. Fast frames keep the core.",
								"Default: on."))
	...
	private static ConfigCategory compat(HSNConfig cfg) {
		boolean vs = hsn.optimizations.client.compat.VoxelSniperCompat.present();
		boolean we = hsn.optimizations.client.compat.VoxelSniperCompat.worldEditPresent();
		String seen = vs && we ? "VoxelSniper + WorldEdit loaded."
				: vs ? "VoxelSniper loaded."
				: we ? "WorldEdit / FAWE loaded."
				: "No editor mod detected. Options stay idle until one is installed.";
		return ConfigCategory.createBuilder()
				.name(Component.literal("Other mods").withStyle(ChatFormatting.GOLD))
				.tooltip(Component.literal("VoxelSniper, WorldEdit, and other-mod hand-offs."))
				.option(label("Editor tools", seen + " Status: " + hsn.optimizations.client.compat.VoxelSniperCompat.status()))
				.group(OptionGroup.createBuilder()
						.name(Component.literal("VoxelSniper").withStyle(ChatFormatting.AQUA))
						.option(toggle("VoxelSniper Integration", () -> cfg.voxelSniperCompatEnabled, v -> cfg.voxelSniperCompatEnabled = v, true,
								"When a sniper or WorldEdit tool is in hand, HSN can pause long-range culls so the aimed block stays drawn.",
								"Default: on. Does nothing if no editor mod is present."))
						.option(toggle("Detect WorldEdit / FAWE", () -> cfg.voxelSniperDetectWorldEdit, v -> cfg.voxelSniperDetectWorldEdit = v, true,
								"Treat WorldEdit / FastAsyncWorldEdit / WorldEdit CUI as editor tools (wooden axe wand).",
								"Default: on."))
						.option(toggle("Always Scan Tools", () -> cfg.voxelSniperAlwaysScanTools, v -> cfg.voxelSniperAlwaysScanTools = v, false,
								"Watch arrow, gunpowder and wand items even if VoxelSniper's mod id was not detected.",
								"Default: off. Use this on unofficial ports."))
						.option(toggle("Pause Horizon Y While Aiming", () -> cfg.voxelSniperPauseHorizon, v -> cfg.voxelSniperPauseHorizon = v, true,
								"Turns off Horizon Y cull while a sniper tool is held so deep target blocks stay visible.",
								"Default: on."))
						.option(toggle("Pause Terrain Mask While Aiming", () -> cfg.voxelSniperPauseTerrainMask, v -> cfg.voxelSniperPauseTerrainMask = v, true,
								"Turns off the circular / hex terrain mask while a sniper tool is held so far corners stay drawn.",
								"Default: on."))
						.option(toggle("Pause Entity Cull While Aiming", () -> cfg.voxelSniperPauseEntityCull, v -> cfg.voxelSniperPauseEntityCull = v, false,
								"Disables HSN entity distance culling while a sniper tool is held. Usually unnecessary.",
								"Default: off."))
						.option(slider("Sniper Keep Range", () -> (int) cfg.voxelSniperToolRange, v -> cfg.voxelSniperToolRange = v, 128, 16, 384, "blocks",
								"Range used when the integration is active. Matches typical VoxelSniper brush reach.",
								"Default 128 blocks."))
						.build())
				.build();
	}

	private static OptionGroup scaleGroup(HSNConfig cfg) {
		boolean blocked = ResolutionControlCompat.present();
		Component title = blocked
				? Component.literal("3D Render Scale").withStyle(ChatFormatting.STRIKETHROUGH, ChatFormatting.DARK_GRAY)
				: Component.literal("3D Render Scale").withStyle(ChatFormatting.AQUA);
		String owner = blocked ? ResolutionControlCompat.ownerLabel() : "";
		var group = OptionGroup.createBuilder().name(title);
		if (blocked) {
			group.option(LabelOption.createBuilder()
					.line(Component.literal("Disabled — Resolution Control++ / RenderScale is in this instance (" + owner + ").")
							.withStyle(ChatFormatting.RED, ChatFormatting.ITALIC))
					.line(Component.literal("HSN will not scale the framebuffer. Use that mod's scale slider instead.")
							.withStyle(ChatFormatting.GRAY, ChatFormatting.STRIKETHROUGH))
					.build());
		} else {
			group.option(LabelOption.createBuilder()
					.line(Component.literal("World resolution only. Hotbar, chat and menus stay sharp.")
							.withStyle(ChatFormatting.GRAY))
					.line(Component.literal("50% is a big FPS lift on integrated GPUs. 100% is vanilla. Sky, fog and terrain stay on this buffer.")
							.withStyle(ChatFormatting.DARK_GREEN))
					.line(Component.literal("Now: " + RenderScale.statusLine()).withStyle(ChatFormatting.AQUA, ChatFormatting.BOLD))
					.build());
		}
		return group
				.option(scaleToggle("Enable 3D Scale", () -> cfg.renderScaleEnabled && !blocked,
						v -> {
							if (blocked) {
						...
						"Turns on an internal world resolution. The HUD never scales with this slider.",
						"Off = vanilla 100%. On + 50–75% is the usual laptop setup."))
				.option(scaleSlider("World Scale",
						() -> (int) Math.round(cfg.renderScale * 100),
						v -> {
							if (blocked) {
								return;
							}
						...
						"100% is native. 50% draws the world at half resolution (more FPS, slightly softer). 200% is a light supersample."))
				.option(Option.<ScaleFilter>createBuilder()
						.name(blocked
								? Component.literal("Scale Filter").withStyle(ChatFormatting.STRIKETHROUGH, ChatFormatting.DARK_GRAY)
								: Component.literal("Scale Filter"))
						.description(desc(
								"How the scaled world image is sampled onto the window.",
								"Linear — smoother upscale / downsample (good above and below native).",
								"Nearest — sharp pixels when rendering below native.",
								blocked ? "Locked because Resolution Control owns this feature." : "Recommended: Linear."))
						.instant(true)
						.available(!blocked)
						.binding(ScaleFilter.LINEAR, () -> cfg.renderScaleFilter == null ? ScaleFilter.LINEAR : cfg.renderScaleFilter, v -> {
							if (!blocked) {
								cfg.renderScaleFilter = v;
							}
						})
						.controller(opt -> EnumControllerBuilder.create(opt).enumClass(ScaleFilter.class))
						.build())
				.option(scaleToggle("Adaptive World Scale", () -> cfg.renderScaleAdaptive && !blocked,
						v -> {
							if (blocked) {
								cfg.renderScaleAdaptive = false;
								return;
							}
							cfg.renderScaleAdaptive = v;
						}, false, blocked,
						"When Adaptive Culling is also on, world scale eases down with FPS. The framebuffer is not rebuilt every tick; the world pass just uses a quantized factor.",
						"Default: off. Turn on together with Adaptive Culling if you want resolution to follow load."))
				.build();
	}

	private static Option<Boolean> scaleToggle(String name, java.util.function.Supplier<Boolean> get, Consumer<Boolean> set,
			boolean def, boolean blocked, String what, String advice) {
		return Option.<Boolean>createBuilder()
				.name(blocked
						? Component.literal(name).withStyle(ChatFormatting.STRIKETHROUGH, ChatFormatting.DARK_GRAY)
						: Component.literal(name))
				.description(desc(what, advice, blocked ? "Disabled by Resolution Control++." : FPS_ON_HIGH))
				.instant(true)
				.available(!blocked)
				.binding(def, get, set)
	...
	private static Option<Integer> scaleSlider(String name, java.util.function.Supplier<Integer> get, Consumer<Integer> set,
			int def, int min, int max, boolean blocked, String what) {
		return Option.<Integer>createBuilder()
				.name(blocked
						? Component.literal(name).withStyle(ChatFormatting.STRIKETHROUGH, ChatFormatting.DARK_GRAY)
						: Component.literal(name))
				.description(desc(what, blocked ? "Locked. Another resolution-scale mod is installed." : "Default 100%.", FPS_LOWER_HIGH))
				.instant(true)
				.available(!blocked)
				.binding(def, get, set)
				.controller(opt -> IntegerSliderControllerBuilder.create(opt)
						.range(min, max)
						.step(5)
						.formatValue(v -> Component.literal(v + " %").withStyle(
								blocked ? ChatFormatting.DARK_GRAY : ChatFormatting.AQUA)))
				.build();
	}
	...
	private static Option<Integer> slider(String name, java.util.function.Supplier<Integer> get, Consumer<Integer> set,
		...
		int safeDef = def < lo ? lo : (def > hi ? hi : def);
		return Option.<Integer>createBuilder()
				.name(Component.literal(name))
				.description(desc(what, advice, fps))
				.instant(true)

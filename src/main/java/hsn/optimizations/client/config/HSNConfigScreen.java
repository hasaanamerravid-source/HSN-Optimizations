...
public final class HSNConfigScreen extends Screen {
	...
	private HSNConfigScreen(Screen parent) {
		super(Component.literal("HSN-Optimizations 4.0"));
		this.parent = parent;
	}
	...
	private void layoutRows() {
		...
		switch (tab) {
			case GENERAL -> {
				y = addToggle(x, y, w, "Enable HSN", "Master switch. Off = vanilla everywhere.",
						c.modEnabled, v -> {
							c.modEnabled = v;
							HotPath.rebuild(c);
						});
				y = addCycle(x, y, w, "Profile", "Writes every distance and quality slider.",
			...
			case RENDERING -> {
				y = addToggle(x, y, w, "World scale", "3D framebuffer only. HUD stays native.",
						c.renderScaleEnabled, v -> c.renderScaleEnabled = v);
				y = addSteps(x, y, w, "Scale percent", (int) Math.round(c.renderScale * 100.0), 25, 200, 5,
				...
				y = addToggle(x, y, w, "Adaptive scale", "Ease world scale toward the FPS target.",
	...
	private int addSteps(int x, int y, int w, String name, int value, int min, int max, int step,
		...
		if (rowVisible(y)) {
			...
			addRenderableWidget(Button.builder(Component.literal("-"), btn -> {
				set.accept(Math.max(min, value - step));
				HSNConfig.get().save();
				rebuild();
			}).bounds(minus, y, 20, 20).build());
			addRenderableWidget(Button.builder(Component.literal(fmt.apply(value)), btn -> {
			}).bounds(minus + 22, y, CTRL - 44, 20)
					.tooltip(Tooltip.create(Component.literal(name + " — use + / − or the mouse wheel on this tab"))).build());
			addRenderableWidget(Button.builder(Component.literal("+"), btn -> {
				set.accept(Math.min(max, value + step));
				HSNConfig.get().save();
				rebuild();
			}).bounds(minus + CTRL - 20, y, 20, 20).build());
		}
		return y + ROW;
	}
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.fill(0, 0, this.width, HEAD, 0xF20C140C);
		graphics.fill(0, HEAD, SIDE, this.height - FOOT, 0xE60C140C);
		graphics.fill(SIDE, HEAD, this.width, this.height - FOOT, 0xD80A100A);
		graphics.fill(0, this.height - FOOT, this.width, this.height, 0xF20C140C);
		graphics.fill(0, HEAD, this.width, HEAD + 1, 0xFF4A7C59);
		graphics.fill(SIDE, HEAD, SIDE + 1, this.height - FOOT, 0xAA4A7C59);
		graphics.fill(0, this.height - FOOT, this.width, this.height - FOOT + 1, 0xFF4A7C59);

		drawMark(graphics, 8, 7);
		graphics.text(this.font, "HSN-Optimizations 4.0", 34, 8, 0xFF8ACC8A, false);
		HSNConfig cfg = HSNConfig.get();
		String status = (cfg.modEnabled ? "On" : "Off") + "  ·  " + labelOf(cfg.lastAppliedPreset);
		graphics.text(this.font, status, 34, 19, 0xFF4A7C59, false);

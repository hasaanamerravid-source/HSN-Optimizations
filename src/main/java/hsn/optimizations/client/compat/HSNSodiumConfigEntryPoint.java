		...
		try {
			...
			OptionGroupBuilder performance = builder.createOptionGroup()
					...
					.addOption(bool(builder, "mod_enabled",
							...
							"Master switch. Off = every HSN pass is a no-op. Bind the optional kill-switch key in Controls.",
							() -> cfg.modEnabled,
					...
					.addOption(bool(builder, "fps_overlay",
							"FPS Overlay (F7)",
							"Show live FPS + cull scale overlay. The settings keybind starts unbound.",
			...
			OptionGroupBuilder terrain = builder.createOptionGroup()
					...
					.addOption(bool(builder, "vert_range_on",
							"Vertical Range Limit",
							"Clip sections far above or below the camera.",
							() -> cfg.circularVerticalRangeEnabled,
							v -> cfg.circularVerticalRangeEnabled = v,
							false))
					.addOption(slider(builder, "vert_range",
							"Vertical Range",
							"Blocks above and below the camera when the vertical limit is on.",
							() -> cfg.circularVerticalRange,
							v -> cfg.circularVerticalRange = v,
							16, 4, 64, 1,
							blocks()))
					.addOption(bool(builder, "horizon_y",
							"Horizon Y Cull",
							"Do not draw sections / entities / particles below the lowest visible Y.",
							() -> cfg.horizonYCullEnabled,
							v -> { cfg.horizonYCullEnabled = v; HotPath.rebuild(cfg); },
							true))
					.addOption(slider(builder, "horizon_keep",
							"Horizon Keep-Below",
							"Blocks kept under the camera so the floor at your feet stays drawn.",
							() -> (int) cfg.horizonYKeepBelow,
							v -> cfg.horizonYKeepBelow = v,
							24, 8, 96, 1,
							blocks()))
					.addOption(slider(builder, "keep_chunks",
							"Always-Keep Chunks",
							"Chunk radius the terrain mask may not hide.",
							() -> cfg.alwaysKeepChunks,
							v -> cfg.alwaysKeepChunks = v,
							3, 2, 8, 1,
							chunks()));

			OptionGroupBuilder scale = builder.createOptionGroup()
					.setName(Component.literal("Render Scale"))
					.addOption(bool(builder, "render_scale_on",
							"Enable 3D Scale",
							"World framebuffer scale. HUD stays native. Off if Resolution Control is loaded.",
							() -> cfg.renderScaleEnabled,
							v -> { cfg.renderScaleEnabled = v; hsn.optimizations.client.optimize.RenderScale.notifyChanged(); },
							false))
					.addOption(slider(builder, "render_scale_pct",
							"World Scale",
							"100 = native. 50 = half-res world.",
							() -> (int) Math.round(cfg.renderScale * 100),
							v -> { cfg.renderScale = v / 100.0; hsn.optimizations.client.optimize.RenderScale.notifyChanged(); },
							100, 25, 200, 5,
							v -> Component.literal(v + "%")))
					.addOption(enumOpt(builder, "render_scale_filter", ScaleFilter.class,
							"Scale Filter",
							"Linear is smoother. Nearest is sharper below native.",
							() -> cfg.renderScaleFilter == null ? ScaleFilter.LINEAR : cfg.renderScaleFilter,
							v -> cfg.renderScaleFilter = v,
							ScaleFilter.LINEAR))
					.addOption(bool(builder, "render_scale_adapt",
							"Adaptive World Scale",
							"Ease world scale with Adaptive Culling. Needs Adaptive Culling on.",
							() -> cfg.renderScaleAdaptive,
							v -> cfg.renderScaleAdaptive = v,
							false));

			OptionGroupBuilder highEnd = builder.createOptionGroup()
					.setName(Component.literal("High-end CPU"))
					.addOption(bool(builder, "lightmap_cache", "Lightmap Cache", "Skip LightTexture rebuilds when nothing changed.", () -> cfg.lightmapCacheEnabled, v -> cfg.lightmapCacheEnabled = v, true))
					.addOption(bool(builder, "interp_skip", "Skip Far Interpolation", "Skip client interpolation for far non-combat entities.", () -> cfg.entityInterpSkipEnabled, v -> cfg.entityInterpSkipEnabled = v, true))
					.addOption(slider(builder, "interp_dist", "Interp Skip Distance", "Distance where interpolation may skip.", () -> (int) cfg.entityInterpSkipDistance, v -> cfg.entityInterpSkipDistance = v, 48, 8, 256, 1, blocks()))
					.addOption(bool(builder, "distant_tick", "Distant Client Tick Skip", "Skip some client ticks on distant decorations.", () -> cfg.distantClientTickSkipEnabled, v -> cfg.distantClientTickSkipEnabled = v, true))
					.addOption(bool(builder, "unfocused_cap", "Unfocused FPS Cap", "Cap FPS when the window is not focused.", () -> cfg.unfocusedFpsCapEnabled, v -> cfg.unfocusedFpsCapEnabled = v, true))
					.addOption(slider(builder, "unfocused_fps", "Unfocused Cap", "FPS cap while unfocused.", () -> cfg.unfocusedFpsCap, v -> cfg.unfocusedFpsCap = v, 30, 5, 240, 1, v -> Component.literal(v + " FPS")))
					.addOption(bool(builder, "cloud_lod", "Cloud LOD", "Cheapen far clouds when not looking up.", () -> cfg.cloudLodEnabled, v -> cfg.cloudLodEnabled = v, true))
					.addOption(bool(builder, "weather_lod", "Weather LOD", "Thin weather work underground or at high FPS.", () -> cfg.weatherRendererLodEnabled, v -> cfg.weatherRendererLodEnabled = v, true))
					.addOption(bool(builder, "living_anim", "Living Anim Throttle", "Skip far limb-swing work.", () -> cfg.livingAnimThrottleEnabled, v -> cfg.livingAnimThrottleEnabled = v, true))
					.addOption(bool(builder, "map_throttle", "Map Renderer Throttle", "Rebuild map textures less often.", () -> cfg.mapRendererThrottleEnabled, v -> cfg.mapRendererThrottleEnabled = v, true))
					.addOption(bool(builder, "sky_extras", "Sky Extras Throttle", "Skip stars / sunrise extras when looking down.", () -> cfg.skyExtrasThrottleEnabled, v -> cfg.skyExtrasThrottleEnabled = v, true))
					.addOption(bool(builder, "firework_cap", "Firework Particle Cap", "Independent firework burst cap.", () -> cfg.fireworkParticleCapEnabled, v -> cfg.fireworkParticleCapEnabled = v, true))
					.addOption(slider(builder, "firework_max", "Max Firework Particles", "Per-tick firework particle cap.", () -> cfg.maxFireworkParticlesPerTick, v -> cfg.maxFireworkParticlesPerTick = v, 48, 4, 256, 4, v -> Component.literal(String.valueOf(v))))
					.addOption(bool(builder, "idle_ai", "Idle AI Throttle", "Skip idle GoalSelector far from players.", () -> cfg.idleAiThrottleEnabled, v -> cfg.idleAiThrottleEnabled = v, true))
					.addOption(bool(builder, "world_border", "World Border LOD", "Skip border mesh when far inside.", () -> cfg.worldBorderLodEnabled, v -> cfg.worldBorderLodEnabled = v, true))
					.addOption(bool(builder, "drip_throttle", "Drip Particle Throttle", "Skip dripping particles underground or far away.", () -> cfg.dripParticleThrottleEnabled, v -> cfg.dripParticleThrottleEnabled = v, true))
					.addOption(bool(builder, "hard_part_cap", "Hard Particle Cap", "Hard-cap live particles each tick.", () -> cfg.hardParticleCapEnabled, v -> cfg.hardParticleCapEnabled = v, true))
					.addOption(bool(builder, "boss_skip", "Skip Empty Boss Overlay", "Skip boss-bar layout when no bosses are active.", () -> cfg.skipEmptyBossOverlayEnabled, v -> cfg.skipEmptyBossOverlayEnabled = v, true))
					.addOption(bool(builder, "tex_lod", "Block Texture LOD", "Mip bias on the blocks atlas during the world pass.", () -> cfg.blockTextureLodEnabled, v -> cfg.blockTextureLodEnabled = v, true))
					.addOption(bool(builder, "name_tags", "Name Tag Cull", "Hide name tags beyond a distance.", () -> cfg.nameTagCullEnabled, v -> cfg.nameTagCullEnabled = v, true))
					.addOption(bool(builder, "behind_cam", "Behind-Camera Cull", "Skip far entities behind the camera.", () -> cfg.behindCameraCullEnabled, v -> { cfg.behindCameraCullEnabled = v; HotPath.rebuild(cfg); }, false));

			OptionGroupBuilder voxel = builder.createOptionGroup()
					.setName(Component.literal("VoxelSniper"))
					.addOption(bool(builder, "vs_compat",
							"VoxelSniper Integration",
							"Pause long-range culls while a sniper or WorldEdit tool is held.",
							() -> cfg.voxelSniperCompatEnabled,
							v -> cfg.voxelSniperCompatEnabled = v,
							true))
					.addOption(bool(builder, "vs_we",
							"Detect WorldEdit / FAWE",
							"Treat WorldEdit wand as an editor tool.",
							() -> cfg.voxelSniperDetectWorldEdit,
							v -> cfg.voxelSniperDetectWorldEdit = v,
							true))
					.addOption(bool(builder, "vs_scan",
							"Always Scan Tools",
							"Watch arrow / gunpowder / wand even if the mod id was missed.",
							() -> cfg.voxelSniperAlwaysScanTools,
							v -> cfg.voxelSniperAlwaysScanTools = v,
							false))
					.addOption(bool(builder, "vs_horizon",
							"Pause Horizon Y While Aiming",
							"Keep deep target blocks visible while sniping.",
							() -> cfg.voxelSniperPauseHorizon,
							v -> cfg.voxelSniperPauseHorizon = v,
							true))
					.addOption(bool(builder, "vs_mask",
							"Pause Terrain Mask While Aiming",
							"Keep far corners drawn while sniping.",
							() -> cfg.voxelSniperPauseTerrainMask,
							v -> cfg.voxelSniperPauseTerrainMask = v,
							true))
					.addOption(bool(builder, "vs_entity",
							"Pause Entity Cull While Aiming",
							"Disable entity distance cull while a tool is held.",
							() -> cfg.voxelSniperPauseEntityCull,
							v -> cfg.voxelSniperPauseEntityCull = v,
							false))
					.addOption(slider(builder, "vs_range",
							"Sniper Keep Range",
							"Typical VoxelSniper brush reach used by the integration.",
							() -> (int) cfg.voxelSniperToolRange,
							v -> cfg.voxelSniperToolRange = v,
							128, 16, 384, 8,
							blocks()));

			OptionGroupBuilder hud = builder.createOptionGroup()
					.setName(Component.literal("Debug / HUD"))
					.addOption(bool(builder, "f3_status", "F3 Status", "HSN block on the F3 right column.", () -> cfg.f3ShowStatus, v -> cfg.f3ShowStatus = v, true))
					.addOption(bool(builder, "f3_compact", "F3 Compact", "Two-line F3 summary.", () -> cfg.f3Compact, v -> cfg.f3Compact = v, true))
					.addOption(bool(builder, "f3_details2", "F3 Details", "Extra distances and flags on F3.", () -> cfg.f3ShowDetails, v -> cfg.f3ShowDetails = v, true))
					.addOption(slider(builder, "overlay_x", "Overlay X", "FPS overlay X offset.", () -> cfg.fpsOverlayX, v -> cfg.fpsOverlayX = v, 4, 0, 400, 1, v -> Component.literal(v + " px")))
					.addOption(slider(builder, "overlay_y", "Overlay Y", "FPS overlay Y offset.", () -> cfg.fpsOverlayY, v -> cfg.fpsOverlayY = v, 4, 0, 400, 1, v -> Component.literal(v + " px")));
			...
			mod.addPage(builder.createOptionPage()
					...
					.addOptionGroup(scale)
					.addOptionGroup(server)
					.addOptionGroup(unique));
			mod.addPage(builder.createOptionPage()
					.setName(Component.literal("HSN High-end"))
					.addOptionGroup(highEnd)
					.addOptionGroup(hud));
			mod.addPage(builder.createOptionPage()
					.setName(Component.literal("HSN Compat"))
					.addOptionGroup(voxel));

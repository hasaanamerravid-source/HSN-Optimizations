...
public final class SodiumOptionsIntegration {
	...
	public static void tryRegister() {
		...
		boolean sodium = HSNPlatform.modPresent("sodium");
		boolean api = isConfigApiPresent();
		if (!sodium) {
			HSNOptimizations.LOGGER.info("Sodium not loaded; HSN settings stay on Mod Menu / the built-in screen.");
			return;
		}
		if (api) {

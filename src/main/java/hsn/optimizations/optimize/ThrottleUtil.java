...
/**
 * Cheap interval helpers for item / XP / AI throttle mixins.
 * Distances are world blocks. Intervals are ticks between full updates.
 */
public final class ThrottleUtil {
	...
	public static int intervalForDistance(double distance, double startDistance, double maxDistance, int maxInterval) {
		...
		return lerpInterval(t, maxInterval);
	}
	...
	private static int lerpInterval(double t, int maxInterval) {
		if (t < 0.0) {
			t = 0.0;
		} else if (t > 1.0) {
			t = 1.0;

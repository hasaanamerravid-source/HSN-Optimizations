	...
	public static final class Scratch implements AutoCloseable {
		...
		public MemorySegment ensure(long bytes) {
			if (!open) {
				...
				open = true;
			} else if (buf.byteSize() < bytes) {
				// Close the old arena to release all prior allocations, then
				// open a fresh one sized for the new request.  Allocating a
				// second segment inside the same arena without closing the
				// first would leak the old backing memory for the lifetime of
				// the arena.
				arena.close();
				arena = Arena.ofConfined();
		...
		public void copyIn(float[] src, int n, MemorySegment dest) {
			...
			n = Math.min(n, Math.min(src.length, (int) Math.min(Integer.MAX_VALUE, dest.byteSize() / Float.BYTES)));
			if (n <= 0) {
				return;
			}
		...
		public void copyOut(MemorySegment src, byte[] dest, int n) {
			...
			n = Math.min(n, Math.min(dest.length, (int) Math.min(Integer.MAX_VALUE, src.byteSize())));
			if (n <= 0) {
				return;
			}

...
public final class WasmSimdKernel {
	...
	private static void runCull(MemorySegment mem, int ptr, int n, double limit, int outPtr) {
		// ptr is the byte offset of the input array; divide once to get the
		// element offset so getAtIndex (which uses element strides) is correct.
		int base = ptr / Double.BYTES;
		int i = 0;
		int bound = n & ~1;
		while (i < bound) {
			double a = mem.getAtIndex(ValueLayout.JAVA_DOUBLE, base + i);
			double b = mem.getAtIndex(ValueLayout.JAVA_DOUBLE, base + i + 1);
	...
	/**
	 * Portable Horizon Y mask. Two-wide compare matching the WASM SIMD
	 * contract; used when no .so exports {@code hsn_horizon_y_f32}.
	 */
	public static boolean horizonY(float[] maxY, float floor, byte[] out, int n) {
		if (maxY == null || out == null || n <= 0) {
			return false;
		}

...
public final class HotPath {
	...
	public static final class Snapshot {
		...
		Snapshot(HSNConfig cfg, double adaptiveScale) {
			...
			if (cfg.modEnabled && (cfg.adaptiveCullingEnabled || cfg.performanceModeEnabled)) {
				s = clamp(adaptiveScale, cfg.adaptiveCullingEnabled ? 0.25 : 0.70, 1.0);
			}
			this.scale = s;
			...
			this.interpDistSq = ExactDistance.limitSq(cfg.entityInterpSkipDistance * s);
			this.clientTickDistSq = ExactDistance.limitSq(cfg.distantClientTickDistance * s);
			this.livingAnimDistSq = ExactDistance.limitSq(cfg.livingAnimThrottleDistance * s);
	...
	public static void publishScale(double next) {
		...
		// Skip rebuild when the scale did not actually move. This is independent
		// of whether adaptive culling is currently on — the previous guard only
		// early-returned when adaptive was enabled, so disabled-path ticks still
		// rebuilt an identical snapshot every frame.
		if (Math.abs(next - SNAP.scale) < 0.005) {
			return;
		}

# HSN 3.8.8 R — polyglot performance pipeline
...
| Batch mask / LOD | Swift | `libhsn_swift.so` | optional sidecar, skipped if no swiftc |
| Batch mask / keep | Haskell | `libhsn_haskell.so` | optional sidecar, skipped if no ghc |
| Extra SoA / rsqrt | AVX2 ASM | same `libhsn_asm.so` | batch of 16+ |
| Extra SoA / morton | C++23 | same `libhsn_cpp.so` | batch of 16+ |
| SPMD cull / rsqrt | ISPC | `libhsn_ispc.so` | `ispc` AVX2, scalar fallback |
| GPU batch sidecar | OpenCL C | `hsn_cull.cl` | large dumps only, never per entity |
| HSNL → C | HSNL + Python | `libhsn_hsnl.so` | compiled offline by `hsnlc.py` |
